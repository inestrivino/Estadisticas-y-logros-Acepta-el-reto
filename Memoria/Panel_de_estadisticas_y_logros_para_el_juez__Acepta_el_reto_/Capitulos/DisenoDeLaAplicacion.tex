...
Según \cite{bartle_players_muds}, los jugadores pueden clasificarse en cuatro tipos no excluyentes, que determinan cómo debe diseñarse la experiencia de juego para poder fidelizarlos. Las cuatro categorías son:
\begin{itemize}
    ...
    \item Competidores (\textit{Killers}): Disfrutan superando a otros, incluso a costa de la colaboración. Es decir, no solo buscan ganar, sino que además buscan hacer perder a otro.
\end{itemize}
...
\begin{figure}
    ...
    \caption[Taxonomía de jugadores de Bartle]{Taxonomía de jugadores de Bartle. Gráfico basado en el diagrama original en ASCII propuesto por Bartle, disponible en Wikipedia: \url{https://en.wikipedia.org/wiki/Bartle_taxonomy_of_player_types#/media/File:Character_theory_chart.svg}}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Categorías: Premian al usuario por hacer ejercicios de determinadas categorías. Las categorías que contemplamos son: construcciones de programación, estructuras de datos, algoritmia, matemáticas, grafos y geometría. 
    \end{itemize}
...
La distribución final de los logros queda establecida en 5 de nivel \textit{Bronce}, 6 de nivel \textit{Plata} y 7 de nivel \textit{Oro}. Esta proporción garantiza un equilibrio en la progresión del usuario, al tiempo que diversifica las categorías de logros según su tipo. Asimismo, se han incluido solo dos logros sorpresa, una decisión que busca satisfacer, en la mayor medida posible, las preferencias de los usuarios encuestados (cuyos resultados pueden leerse en el Apéndice \ref{FormularioJugadores}), cuyas opiniones sobre este aspecto estuvieron divididas.
...
Para los usuarios se han ideado las siguientes estadísticas, considerándolas informativas para entender rápidamente el nivel de experiencia y conocimiento de un determinado usuario, o para servir de referencia a la hora de intentar obtener ciertos logros:
...
Aprendiz     & 0 - 499 XP \\
\midrule
Competente   & 500 - 999 XP \\
\midrule
Hábil        & 1000 - 1,999 XP \\
\midrule
Especialista & 2000 - 4999 XP \\
\midrule
Profesional      & Más de 5000 XP \\
\bottomrule
\end{tabular}
...
Está planteado de tal manera que sea relativamente fácil abandonar los primeros niveles, pero la progresión sea más compleja con el tiempo. Por ejemplo, para pasar de aprendiz a competente se necesitan solo 100 puntos, mientras que para pasar de especialista a profesional se necesitan 1000. 
...
Una característica importante a tener en cuenta tanto en el diseño como la evaluación de un sistema de gamificación es el problema de \textbf{la superación del jefe final}. Este problema se refiere a cuando un sistema de gamificación, sobre todo uno que se introduce en en un sistema ya existente con años de existencia como es \aer, se vuelve redundante porque un número considerable de usuarios obtienen el máximo nivel de dominio, todos los logros, etc. Se pierde la fuente de motivación extrínseca y se inutiliza el esfuerzo invertido en el sistema. Para evitar esto, todas las mecánicas del sistema de gamificación deben ser fácilmente extensibles. 
...
\begin{itemize}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{Imagenes/Bitmap/wireInicio.jpeg}
    ...
    \end{figure}
    \item Tabla de clasificación: Indica al usuario cuál es su estado respecto a los demás usuarios del sistema, tanto a nivel general, como dentro del rango de XP de su nivel. Su \textit{wireframe} se puede ver en la figura \ref{fig:wireClasificacion}.
    \begin{figure}
        \centering
...
\begin{enumerate}
    ...
    \item Como conluye el estudio realizado en \cite{SaezLopez_SevillanoGarcia_VazquezCano_2019}, los estudiantes españoles son más propensos a optar por un portátil cuando se trata de realizar tareas académicas o complejas, categoría que engloba el uso de jueces en línea. Además, el ordenador es la plataforma preferible para la programación debido a sus cualidades (pantalla ancha y teclado) y la elegida para la participación en clases o concursos de programación competitiva. Además, se dispone de los datos de dispositivos para el acceso a \aer del último año (desde la fecha del 17 de mayo de 2026) que se muestran en la figura \ref{fig:sistemasUsoAER}, que demuestran que la gran mayoría de los accesos se realizaron por ordenador. Teniendo todos estos datos en cuenta, se puede determinar que el diseño en móvil, aunque debe existir y ser funcional, no es una prioridad. 
\end{enumerate}
\begin{figure}
        \centering
        \includegraphics[width=0.8\linewidth]{Imagenes/Bitmap/sistemasUsoAER.png}
        \caption{Imágen de los sistemas usados para el acceso a \aer}
        \label{fig:sistemasUsoAER}
    \end{figure}
...
Los wireframes iniciales aquí mostrados sirvieron como base, pero más adelante durante el desarrollo se realizaron ajustes para adaptarlos a la realidad técnica y a feedback de usuarios (descritos más en detalle en los capítulos \ref{cap:implementacion} y \ref{cap:evaluacion}). Es decir, su uso fue orientativo en lugar de restrictivo. 

\section{Arquitectura a alto nivel}

La arquitectura de la aplicación se ha diseñado con el objetivo de proporcionar un sistema modular, escalable y mantenible capaz de satisfacer tanto los requisitos funcionales como los no funcionales definidos en el apéndice de la \ref{cap:SRS}. En particular, la arquitectura busca garantizar la actualización automática de estadísticas, la correcta gestión de usuarios y ejercicios, así como la capacidad de procesar grandes volúmenes de envíos procedentes del juez de programación de forma eficiente y consistente en el tiempo.

Para ello, el sistema se ha estructurado siguiendo una separación clara de responsabilidades entre los distintos componentes principales: frontend, \textit{backend}, base de datos e integración con \aer. Esta separación facilita el mantenimiento del código, la reutilización de componentes y la posibilidad de extender el sistema en el futuro sin necesidad de rediseñar completamente la aplicación.

Además, durante el diseño se han seguido diversos principios de ingeniería software, entre los que destacan:

\begin{itemize}
    \item \textbf{Separación de responsabilidades:} Gracias al uso de una estructura de servicios para gestionar la lógica de negocio, cada componente del sistema tiene una función claramente definida. El frontend se encarga únicamente de la presentación de datos, mientras que el backend procesa la lógica y los DAOs (\textit{Data Access Object}) gestionan el acceso a la base de datos.

    \item \textbf{Modularidad y extensibilidad:} La aplicación está organizada en módulos independientes, lo que facilita añadir nuevas funcionalidades o modificar componentes existentes sin afectar al resto del sistema. Esto resulta especialmente útil en los actualizadores de estado del sistema y registradores de este estado en la base de datos, donde pueden incorporarse nuevas estrategias de procesamiento de manera sencilla. 

    \item \textbf{Bajo acoplamiento entre componentes:} Los distintos elementos de la arquitectura interactúan mediante interfaces bien definidas y mecanismos desacoplados, como APIs o \textit{sockets}. Esto reduce las dependencias directas entre módulos y mejora la mantenibilidad del sistema.

    \item \textbf{Actualización reactiva basada en eventos:} El sistema utiliza eventos y notificaciones para reaccionar automáticamente ante nuevos envíos o cambios en el estado de los usuarios y ejercicios en \aer. Gracias al uso de colas de mensajes, el frontend puede actualizar la información en tiempo real sin intervención manual del usuario.

    \item \textbf{Escalabilidad para el procesamiento de grandes cantidades de envíos:} El procesamiento de envíos se realiza de forma incremental y en bloques de hasta 1000 elementos, evitando sobrecargas de memoria y permitiendo gestionar grandes volúmenes de información de manera eficiente.
\end{itemize}

A alto nivel, la aplicación sigue una arquitectura basada en una SPA (\textit{Single Page Application}). La navegación entre las diferentes ``vistas'' de la página solo implica un cambio en lo que se muestra y una solicitud para obtener los datos que se quieren representar, en lugar de la carga de un nuevo documento HTML.

Esto tiene varias ventajas a nivel de rendimiento, como la reducción del tráfico innecesario entre cliente y servidor, menores tiempos de respuesta tras la carga inicial y una experiencia de usuario más fluida y reactiva, ya que únicamente se actualizan los componentes necesarios de la interfaz\footnote{Para leer una introducción al concepto de las SPAs y sus beneficios: \url{https://www.geeksforgeeks.org/javascript/what-is-single-page-application/}}.


De manera consistente con un sistema modular, el \textit{frontend} se encarga exclusivamente de la visualización y de la interacción con el usuario, mientras que el \textit{backend} concentra toda la lógica de negocio y el procesamiento de datos. Mantiene una comunicación reactiva con el \textit{backend} mediante \textit{sockets}. Cuando el \textit{backend} detecta la llegada de nuevos envíos o cambios relevantes en las estadísticas, envía una notificación al \textit{frontend} para actualizar automáticamente la información mostrada al usuario. Este mecanismo resulta fundamental para satisfacer los requisitos de actualización automática definidos en la especificación funcional.
El \textit{backend} por lo tanto funciona de manera similar a una API, recibiendo solicitudes de la capa \textit{frontend} que responde con información estructurada. Además, maneja la lógica de negocio, como la comprobación de las precondiciones para que un usuario reciba un logro.

Para acceder a la base de datos se utiliza el patrón DAO (\textit{Data Access Object}). En este modelo, los servicios no interactúan directamente con la base de datos, sino que delegan las operaciones de almacenamiento y recuperación de información a objetos DAO especializados. De esta manera, se desacopla la lógica de negocio de la lógica de persistencia, mejorando la mantenibilidad y facilitando posibles cambios futuros en la tecnología de almacenamiento.

Uno de los componentes más interesantes al diseñar la arquitectura, es el sistema de integración con el juez externo, ya que esto determina en gran medida el rendimiento y consistencia de la aplicación. Debido a limitaciones temporales para poder establecer un sistema de mensajería integrado entre \aer y el presente proyecto, se desarrolló como solución temporal un adaptador intermedio basado en \textit{long polling}\footnote{Para aprender sobre las conexiones \textit{long polling}: \url{https://www.geeksforgeeks.org/javascript/what-is-long-polling-and-short-polling/}}. Este adaptador permanece continuamente consultando el endpoint de eventos proporcionado por el juez. Cuando detecta cambios o nuevos envíos, genera un evento. Posteriormente, un componente consumidor dentro del backend recibe estos mensajes y desencadena el procesamiento correspondiente.

La figura \ref{fig:arquitectura_general} muestra una visión general de la arquitectura del sistema y del flujo de datos entre sus componentes principales.

\begin{figure}[H]
\centering
\includegraphics[width=1.2\linewidth]{Imagenes/Bitmap/diagramadearquitectura.drawio.pdf}
\caption{Arquitectura general de alto nivel del sistema}
\label{fig:arquitectura_general}
\end{figure}

Durante este procesamiento se analiza qué usuarios y ejercicios han sido afectados y se recupera su estado actual desde la base de datos. Posteriormente, se aplican las actualizaciones necesarias mediante un conjunto de actualizadores especializados.

Este enfoque permite implementar distintos tipos de actualizadores reutilizando una misma estructura general, favoreciendo la extensibilidad del sistema. El mismo patrón conceptual se reutiliza también en los registradores encargados de persistir la información actualizada.

Dentro del sistema se emplean además diversos patrones de diseño relevantes:

\begin{itemize}
    \item Patrón DAO para el acceso desacoplado a la base de datos.
    \item Patrón Observer en la comunicación reactiva mediante \textit{sockets} y eventos.
    \item Arquitectura MVC implícita derivada de la separación entre frontend, servicios y persistencia.
    \item Patrón Singleton en servicios, routers y DAOs, dado que las instancias exportadas son únicas dentro de la aplicación.
    \item Patrón Strategy en los registradores, los logros y las estadísticas de experiencia, permitiendo añadir nuevas implementaciones sin modificar el código existente.
    \item Patrón Template Method en los actualizadores de estado, donde la clase base fija el flujo del algoritmo de inicialización y las subclases implementan los pasos específicos para cada estadística. Estos patrones se describen en detalle en la sección~\ref{sec:patronesDiseno}.
\end{itemize}

Esta arquitectura permite satisfacer adecuadamente los requisitos funcionales definidos. La separación entre frontend y backend facilita la visualización eficiente de estadísticas de usuarios y ejercicios (REQ-FUN-001 y REQ-FUN-002), mientras que el uso de \textit{sockets} y procesamiento basado en eventos garantiza la actualización automática de información y clasificaciones (REQ-FUN-003 y REQ-FUN-006).

Asimismo, la modularidad de los servicios de procesamiento permite implementar fácilmente sistemas de logros y reglas de puntuación (REQ-FUN-004 y REQ-FUN-005), ya que cada envío puede desencadenar múltiples procesos independientes sobre el estado del usuario. 

Desde el punto de vista de los requisitos no funcionales, la arquitectura propuesta también aporta ventajas importantes. En relación con el rendimiento (REQ-NF-001), gracias al uso de la arquitectura SPA anteriormente mencionada, así como la organización de la recepción y procesamiento de datos, lo cual aporta también al cumplimiento del requisito de fiabilidad (REQ-NF-002). 

En cuanto a la usabilidad (REQ-NF-003), el diseño del frontend permite ofrecer una experiencia de usuario más fluida. La actualización automática de estadísticas y clasificaciones evita que el usuario tenga que recargar manualmente la página para obtener información actualizada.

Finalmente, la arquitectura satisface el requisito de integración con el OJ (REQ-NF-004) gracias a la capa de integración descrita en el corto plazo, y a su estructura modular que favorece la facilidad en las refactorizaciones que sean necesarias para convertirla en un servicio nativo de \aer en el largo plazo. 