...
% Flujo principal: los Pasos (Cajas)
...
% Flechas principales (verticales) con cabezas visibles
\draw[arrow] (p1) -- (p2);
\draw[arrow] (p2) -- (p3);
\draw[arrow] (p3) -- (p4);
\draw[arrow] (p4) -- (p5);

% Iteración por error (No aceptado): De 5 a 2
\draw[arrow, red!70!black]
    (p5.west) 
    .. controls +(-3.5,-0.5) and +(-3.5,0.5) .. 
    (p2.west)
    node [midway, left, align=center, draw=none, black] 
    {\textbf{Reintentar}\\[1mm]WA, TLE,\\etc.};

% Retorno tras Aceptado (AC): De 5 a 1 para empezar de nuevo
% Usamos `looseness=1.5` para dar espacio al texto del "Reintentar"
\draw[arrow, blue!70!black]
    (p5.east) 
    .. controls +(4,0) and +(4,0) .. 
    (p1.east)
    node [midway, right, align=center, draw=none, black] 
    {\textbf{AC} (Aceptada)\\[1mm]¡Problema\\completado!};
...
El concepto de la gamificación ha emergido a partir de comienzos del siglo XXI como un campo interdisciplinario que combina principios de diseño de juegos, psicología motivacional y análisis de comportamiento humano para enriquecer la interacción con sistemas digitales\footnote{Se recomienda leer la siguiente presentación realizada por Amy Jo Kim en 2008, en la que se muestra claramente el panorama de la gamificación en la decada de los 2000: \url{https://es.slideshare.net/slideshow/putting-the-fun-in-functiona/300487\#1}}. En términos generales, la gamificación se define como la utilización de elementos y mecánicas de juego con el propósito de motivar a los usuarios, aumentar su participación y ayudarles a resolver problemas \citep{Zichermann_Cunningham_2011}.  
...
\begin{enumerate}
    ...
    \item  \textbf{Jueces educativos}: Estos sistemas aplican la gamificación para reforzar el aprendizaje individual y mantener la motivación del estudiante. Es habitual el uso de insignias (\textit{badges}) y logros, que recompensan tanto la resolución de ejercicios como la consistencia y el uso de diferentes lenguajes de programación. Plataformas como HackerRank emplean estas insignias visuales para ayudar al usuario a establecer metas y facilitar la consecución de estas\footnote{\url{https://www.hackerrank.com/scoring/badges-and-medals}}, como se muestra en la figura \ref{fig:Insignias disponibles en HackerRank}.
    \begin{figure}
        \centering
...
\midrule
Reclutamiento & Mostrar las habilidades técnicas & Insignias, logros y estadísticas de progreso. \\
\bottomrule
\end{tabularx}
\caption{Resumen de categorías de jueces en línea y sus elementos de gamificación}
\label{tab:Comparación de tipos de OJ}
\end{table}
...
% 1. Caja de la Parte Web (Corregido 'draw=')
\node (web) [fill=blue!5, draw=blue!40!black, minimum width=4.5cm] {
    \textbf{Parte Web}\\
    \small Frontend y Backend\\
    \footnotesize (Interacción, estadísticas, envíos)
};

% 2. Caja de la Base de Datos (Corregido 'draw=')
\node (db) [below=of web, fill=gray!10, draw=gray!40!black, minimum width=4.5cm, minimum height=1.2cm] {
    \textbf{Base de Datos}\\
    \footnotesize (Usuarios, problemas, registros)
};

% 3. Caja del Subsistema de Evaluación (Corregido 'draw=')
\node (juez) [right=of web, fill=red!5, draw=red!40!black, minimum width=4.5cm] {
    \textbf{Subsistema de Evaluación}\\
    \small Demonios de evaluación\\
    \footnotesize (Sandbox, aislamiento, asíncrono)
};

% --- Conexiones y flujos ---
% Relación Web <-> Base de Datos
\draw[biarrow, gray!80!black] (web) -- (db)
    node[midway, right, font=\footnotesize, black, draw=none, fill=none] {Consulta y guarda};

% Envío de código (Web -> Juez)
\draw[arrow, blue!70!black] ([yshift=0.3cm]web.east) -- ([yshift=0.3cm]juez.west)
    node[midway, above, font=\footnotesize, black, draw=none, fill=none] {Envío de código};

% Retorno de veredicto (Juez -> Web)
\draw[arrow, red!70!black] ([yshift=-0.3cm]juez.west) -- ([yshift=-0.3cm]web.east)
    node[midway, below, font=\footnotesize, black, draw=none, fill=none] {Veredicto (asíncrono)};

\end{tikzpicture}
\caption[Arquitectura general del juez en línea]{Arquitectura general y componentes de alto nivel del juez en línea.}
\label{fig:arquitectura_juez}
\end{figure}

\section{\aer}

El juez en línea \aer\ constituye una de las plataformas educativas de referencia en el ámbito hispanohablante para el aprendizaje y la práctica de la programación competitiva. Con un propósito educativo, el sistema ya incorpora diversas herramientas de gamificación orientadas al seguimiento del progreso individual del estudiante\footnote{\url{https://aceptaelreto.com/}}. 

Como se ilustra en la Figura \ref{fig:Estadísticas de Acepta el Reto}, el perfil de usuario contiene métricas clave para el autoaprendizaje, entre las que destacan la cantidad de envíos realizados, correctos e intentados, el volumen total de ejercicios resueltos, así como gráficos distributivos de los lenguajes de programación más utilizados y los veredictos más frecuentes.

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{Imagenes//Bitmap/EstadisticasAceptaElReto.png}
    \caption{Estadísticas de \aer}
    \label{fig:Estadísticas de Acepta el Reto}
\end{figure}

Estos componentes permiten a los usuarios monitorizar su evolución temporal y tomar decisiones sobre a qué área focalizar su progreso, aunque de manera limitada. Complementariamente, la plataforma ofrece un módulo de acceso público que monitoriza de forma continua los últimos envíos procesados por el sistema\footnote{\url{https://aceptaelreto.com/statistics/submissions.php}}, aunque no muestra estadísticas ni datos globales de estos envíos.

Desde el punto de vista de la arquitectura, la arquitectura de \aer, detallada en \cite{Pedro_Pablo_Gomez_Martin_Marco_Antonio_Gomez_Martin_2017}, traslada la separación de responsabilidades (web y evaluación) mediante una infraestructura híbrida y singular, fundamentada en una combinación de tecnologías clásicas.

En lo que respecta a \textbf{componentes web}, la plataforma segmenta su lógica:

\begin{itemize}
    \item \textbf{Capa de interacción y gestión de usuarios:} Implementada principalmente en PHP, se encarga del renderizado de la interfaz de usuario tradicional, la navegación por el catálogo de problemas y la consulta pública de envíos.
    \item \textbf{Capa de lógica de negocio interna:} Delegada en un contenedor de servlets Apache Tomcat (Java).
    \item \textbf{Servicio web interno de eventos}: Para la obtención de datos sobre los envíos y sus resultados en tiempo real. Este servicio es de especial importancia para el desarrollo de este proyecto.
    \item \textbf{Evaluador}: Reciben los envíos de los usuarios y los comprueban en un sandbox para después devolver el resultado obtenido.
\end{itemize}

El nexo de unión de esta infraestructura web es un sistema de gestión de bases de datos MySQL, el cual actúa como repositorio centralizado de persistencia para los datos de la aplicación.

\section{Conclusiones}

El análisis del estado del arte muestra que la visualización de estadísticas y la gamificación constituyen componentes fundamentales en los jueces en línea modernos, especialmente en aquellos con fines educativos y de reclutamiento. La presentación clara de métricas como envíos correctos, problemas resueltos, veredictos obtenidos y logros alcanzados permite a los usuarios evaluar su progreso, identificar áreas de mejora y mantener la motivación a lo largo del tiempo. Por tanto, la creación de pantallas de estadísticas y logros no solo mejora la experiencia del usuario, sino que constituye un elemento esencial para el aprendizaje, la consistencia y la participación activa en la plataforma. Además, para acceder a estos datos de manera consistente, es importante contar con una arquitectura fiable a la vez que rápida.